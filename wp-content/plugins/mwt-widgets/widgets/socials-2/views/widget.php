...
if ( defined( 'FW' ) ) {
	...
	if ( ! empty ( $params['social_icons'] ) ) : ?>
		<div class="widget-socials-2 no-bullets no-top-border no-bottom-border topmargin_0">
                ...
                <?php foreach ( $params['social_icons'] as $icon ): ?>
                    <li class="list-item">
                        <?php if ( ! empty ( $icon['icon_class'] ) && ! empty ( $icon['icon_link'] )  ): ?>
                            ...
                            <a href="<?php echo esc_url( $icon['icon_link'] ); ?>" target="_blank" class="icon-title"><?php echo esc_html( $icon['icon_title'] ); ?></a>
                        <?php endif; //icon	?>
                    </li>
                <?php endforeach; ?>
            </ul>
